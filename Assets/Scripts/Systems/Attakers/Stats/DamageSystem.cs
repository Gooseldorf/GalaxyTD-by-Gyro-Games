...
public static class DamageSystem
{
    ...
    public static void DoDamageByWall(EntityManager manager, KnockBackWallDamageEvent wallDamageEvent, RefRW<CashComponent> cashComponent, EntityCommandBuffer ecb)
    {
        ...
        {
            damage = GetKnockBackDamage(wallDamageEvent.Speed, creepComponent.Mass);
    ...
    public static void DoInstantKill(ref CreepComponent creepComponent, float2 position, EntityCommandBuffer ecb) //bool isByWall ??
    {
        float damage = creepComponent.Hp + creepComponent.MaxHp; // here is damage to make creeps HP = -MaxHP for damageText
    ...
    private static void OnDoDamageToCreep(ref CreepComponent creepComponent, AllEnums.FleshType fleshType, float2 knockbackDirection, float damage, EntityManager manager, Entity creepEntity,
    ...
    {
        var destroyComponent = manager.GetComponentData<DestroyComponent>(creep);
        if (!destroyComponent.IsNeedToDestroy)
